...
public sealed class ProfileEndpointTests : IClassFixture<ProfileDatabaseFixture>
{
    ...
    private static async Task<CookieValue> LoginAsync(
        HttpClient client,
        ...
        object? body = null)
    {
        ...
        return await client.SendAsync(WithCookie(request, session));
    }

    private static HttpRequestMessage WithCookie(HttpRequestMessage request, CookieValue session)
    {
        request.Headers.Add("Cookie", session.Pair);
        return request;
    }
