...
| TM-15 | I | APIs, logs, traces, cache, or integration events expose more profile/member data than the consumer requires. | High | Explicit DTO and contract allowlists, field-level authorization, data-classification review, log redaction, no full entity serialization; retain only the Discord ID/display-name participant snapshot needed for match provisioning; response and telemetry snapshot tests. | Planned |
